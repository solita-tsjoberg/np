...
predictions_num <- predictions %>%
  as_tibble() %>%
  mutate(pred_num=max.col(predictions)-1) %>%
  select(pred_num)
```
...
```{r}
rand_num <- sample(1:nrow(test_labels), size = 1)
cat('test bild nummer', rand_num, 'som föreställer siffran', test_labels[rand_num], ':\n\n')
cat('och modellen predicerar', predictions_num[[rand_num, 1]])
